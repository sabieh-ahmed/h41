...
class Languages extends \G2\L\Controller {
	...
	function index(){
		if($this->data('lang')){
			\G2\L\Lang::read(\G2\Globals::ext_path('chronoforums', 'admin'), $this->data('lang'));
		}
		...
		if((isset($this->data['save']) OR isset($this->data['update'])) AND $this->data('lang')){
			if(isset($this->data['save'])){
				$path = \G2\Globals::ext_path('chronoforums', 'admin').'locales'.DS.$this->data('lang').'.custom.ini';
			}else{
				$path = \G2\Globals::ext_path('chronoforums', 'admin').'locales'.DS.$this->data('lang').'.ini';
			}
			...
			if($result === true){
				...
				return ['error' => rl('Error saving the language file.'), 'reload' => true];
			}
		}
	}
	
	function build(){
		$path = \G2\Globals::ext_path('chronoforums', 'admin');
		$files = \G2\L\Folder::getFiles($path, true);
		$path = \G2\Globals::ext_path('chronoforums', 'front');
