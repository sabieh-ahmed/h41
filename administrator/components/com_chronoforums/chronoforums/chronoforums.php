...
class Chronoforums extends \GCore\Libs\GController {
	...
	function permissions(){
		$perms = array(
			'display' => l_('PERMS_DISPLAY_ACCESS'),
			'make_topics' => l_('PERMS_CREATE_TOPICS'),
			'read_topics' => l_('PERMS_VIEW_TOPICS'),
			'make_posts' => l_('PERMS_REPLY'),
			'modify_topics' => l_('PERMS_MODIFY_TOPICS'),
			'list_attachments' => l_('PERMS_VIEW_ATTACH'),
			'download_attachments' => l_('PERMS_DOWNLOAD_ATTACHMENTS'),
			'attach_files' => l_('PERMS_ATTACH'),
			'edit_posts' => l_('PERMS_EDIT_POSTS'),
			'delete_posts' => l_('PERMS_DELETE_POSTS'),
			'report_posts' => l_('PERMS_REPORT_POSTS'),
			'view_reports' => l_('PERMS_VIEW_REPORTS'),
			'select_answers' => l_('PERMS_SELECT_ANSWER'),
			'feature_topics' => l_('PERMS_FEATURE'),
			'make_votes' => l_('PERMS_VOTE'),
			'view_private' => l_('PERMS_VIEW_PRIVATE'),
		);
		$this->set('perms', $perms);
		
		//$rules = $this->Group->find('list', array('fields' => array('id', 'title')));
		$this->Group->parent_id = 'parent_id';
		$groups = $this->Group->find('flat', array('fields' => array('id', 'title', 'parent_id')));
		$rules = array();
		foreach($groups as $k => $group){
			$rules[$group['Group']['id']] = str_repeat('- ', $group['Group']['_depth']).$group['Group']['title'];
		}
		$rules['owner'] = 'Owner';
	...
	function install_locale(){
		...
		if(isset($_FILES['upload'])){
			...
			if(\GCore\Libs\Upload::valid($upload) AND \GCore\Libs\Upload::not_empty($upload) AND \GCore\Libs\Upload::check_type($upload, 'zip')){
				...
				if(empty($result)){
					$session->setFlash('error', l_('CFU_LOCALE_UPLOAD_ERROR'));
					$this->redirect(r_('index.php?ext=chronoforums&act=install_locale'));
				}
				//file upload, let's extract it
				$zip = new \ZipArchive();
				$handler = $zip->open($target);
				if($handler === true){
					$extract_path = \GCore\C::ext_path('chronoforums', \GCore\Libs\Request::data('target', 'admin')).'locales'.DS;
					$zip->extractTo($extract_path);
					$zip->close();
					unlink($target);
					
					$session->setFlash('success', l_('CFU_LOCALE_INSTALL_SUCCESS'));
					$this->redirect(r_('index.php?ext=chronoforums'));
				}else{
					$session->setFlash('error', l_('CFU_LOCALE_EXTRACT_ERROR'));
					$this->redirect(r_('index.php?ext=chronoforums&act=install_locale'));
				}
			}else{
				$session->setFlash('error', l_('CFU_LOCALE_FILE_ERROR'));
				$this->redirect(r_('index.php?ext=chronoforums&act=install_locale'));
			}
		}
	}
