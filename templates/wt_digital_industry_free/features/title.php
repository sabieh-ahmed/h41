	...
	public function renderFeature() {
		...
		if($menuitem) {
			...
			if($params->get('enable_page_title', 0)) {
				...
				$output .= '<div class="col-md-6 text-md-left"><h2 class="cornered-heading">'. $page_title .'</h2>';

				if($page_subtitle) {
					$output .= '<h3>'. $page_subtitle .'</h3></div>';
				}
				$output .= '</div>';
				$output .= '<div class="col-md-6 text-md-right"><jdoc:include type="modules" name="breadcrumb" style="none" /></div>';
