...
class TopicTasks {
	...
	function tasks_list($topic = array()){
		//echo '<form action="'.r_('index.php?ext=chronoforums&cont=topics').'" method="post" name="searchform">';
	...
	function tasks_form($topic = array()){
		if(!is_null($topic) AND \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics')){
			echo '<form action="'.r_('index.php?ext=chronoforums&cont=topics').'" method="post" name="topictasksform" class="form-inline">';
			...
			$doc->addJsCode('jQuery(document).ready(function($){ $("#topic_tags").select2(
				{
					...
					tokenSeparators: [","," "],
					ajax:{
						url: "'.r_('index.php?ext=chronoforums&cont=topics&act=tags_lookup&tvout=ajax').'",
						dataType: "json",
	...
	function tags_form($topic = array(), $tags = array()){
		...
		?>
		<form action="<?php echo r_('index.php?ext=chronoforums&cont=topics'); ?>" method="post" name="topictagsform">
	...
	function tags_list($topic = array()){
		if(!empty($topic['Tag'])){
			...
			foreach($tags as $tag){
				echo '<a class="cfu-tag cfu-tag-'.$tag.' btn btn-info btn-xs" href="'.r_('index.php?ext=chronoforums&cont=forums&list=tagged&tag='.$tag).'"><span class=""><i class="fa fa-tag fa-lg fa-fw"></i>'.$tag.'</span></a>';
			}
		}
	}
	
	function quick_reply($topic = array()){
		...
		?>
		<form action="<?php echo r_('index.php?ext=chronoforums&cont=posts&act=reply'); ?>" method="post" name="postform" id="postform">
			<div id="quick_reply_preview" class="well" style="display:none;"></div>
			<br />
			<?php $this->view->Bbeditor->editor('Post[text]', $fparams->get('quickreply_textarea_rows', 10)); ?>
			<div class="row cfu-submit text-center">
				<?php echo \GCore\Helpers\Html::input('quick_reply_full', array('type' => 'custom', 'code' => '<a href="'.r_('index.php?ext=chronoforums&cont=posts&act=reply&f='.\GCore\Libs\Request::data('f').'&t='.\GCore\Libs\Request::data('t')).'" class="btn btn-primary btn">'.l_('CHRONOFORUMS_ATTACH_FILES').'</a>')); ?>
				<?php echo \GCore\Helpers\Html::input('quick_reply_preview', array('type' => 'button', 'value' => l_('CHRONOFORUMS_PREVIEW'), 'onclick' => 'preview_quick_reply();', 'class' => 'btn btn-info btn-sm', 'id' => 'quick_reply_preview_button')); ?>
	...
	function reply_button($topic = array()){
		...
		<?php if(empty($topic['Topic']['locked'])): ?>
			<?php if((bool)$fparams->get('hide_post_reply_button', 0) === false OR \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_posts') === true): ?>
				<a title="<?php echo l_('CHRONOFORUMS_POST_REPLY'); ?>" class="btn btn-primary btn-xs cfu-btn-reply gcoreTooltip" href="<?php echo r_('index.php?ext=chronoforums&cont=posts&act=reply&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id']); ?>"><i class="fa fa-fw fa-2x fa-reply"></i><?php //echo l_('CHRONOFORUMS_REPLY'); ?></a>
			<?php endif; ?>
		<?php endif; ?>
		<?php
	}
	
	function subscribe_button($topic = array(), $subscribed = false){
		$fparams = $this->view->vars['fparams'];
		?>
		<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_posts')): ?>
			<?php if($subscribed): ?>
			<a class="btn btn-success active btn-xs gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_SUBSCRIBED_UNSUBSCRIBE_TOPIC'); ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=unsubscribe&t=".$topic['Topic']['id']); ?>"><i class="fa fa-fw fa-2x fa-check-square-o"></i></a>
			<?php else: ?>
			<a class="btn btn-default btn-xs gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_UNSUBSCRIBED_SUBSCRIBE_TOPIC'); ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=subscribe&t=".$topic['Topic']['id']); ?>"><i class="fa fa-fw fa-2x fa-check-square-o"></i></a>
			<?php endif; ?>
		<?php endif; ?>
		<?php
	}
	...
	function new_topic_button($forum = array()){
		...
		<?php if((bool)$fparams->get('hide_new_topic_button', 0) === false OR \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_topics') === true): ?>
			<a class="btn btn-primary btn-xs cfu-btn-new-topic" href="<?php echo r_('index.php?ext=chronoforums&cont=topics&act=add'.(!empty($forum['Forum']['id']) ? '&f='.$forum['Forum']['id'] : '')); ?>"><i class="fa fa-fw fa-2x- fa-plus"></i><?php echo l_('CHRONOFORUMS_NEW_TOPIC'); ?></a>
		<?php endif; ?>
		<?php
	}
	...
	function favorite_button($topic = array()){
		...
		?>
		<a title="<?php echo $title; ?>" class="btn btn-default btn-xs gcoreTooltip" href="<?php echo r_('index.php?ext=chronoforums&cont=topics&act='.$act.'&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id']); ?>"><i class="fa fa-fw fa-2x<?php echo $class; ?>"></i></a>
		<?php
		endif;
	}
	...
	function forum_title($forum = array()){
		?>
		<a class="cfu-title" href="<?php echo r_('index.php?ext=chronoforums&cont=forums&f='.$forum['Forum']['id'].'&alias='.$forum['Forum']['alias']); ?>"><?php echo $forum['Forum']['title']; ?></a>
		<?php
	}
	...
	function search_title(){
		...
		$fparams = $this->view->vars['fparams'];
		$url = r_('index.php?ext=chronoforums&cont=forums'.(!empty($forum) ? '&f='.$forum['Forum']['id'] : ''));
	...
	function search_form($forum = array()){
		$btn_class_search = $btn_class_reset = 'btn-default';
		if(\GCore\Libs\Request::data('keywords')){
			$btn_class_search = 'btn-success active';
			$btn_class_reset = 'btn-default';
		}
		?>
		<form action="<?php echo r_('index.php?ext=chronoforums&cont=forums&f='.$forum['Forum']['id']); ?>" method="post" name="searchform">
			<div class="input-group input-group-sm">
				...
				<span class="input-group-btn">
					<button class="btn <?php echo $btn_class_search; ?>" name="search_posts" type="submit" value=""><i class="fa fa-search"></i></button>
					<button class="btn <?php echo $btn_class_reset; ?> reset" name="reset" type="submit" value=""><i class="fa fa-times"></i></button>
				</span>
			</div>
		</form>
		<?php
	...
	function datetime_icon($topic = array()){
		...
		<?php /*if(!empty($topic['Topic']['created'])): ?>
		<span class="label label-primary"><i class="fa fa-clock-o fa-lg"></i>&nbsp;<?php echo \GCore\Libs\Date::ago(strtotime($topic['Topic']['created'])); ?></span>
		<?php endif;*/ ?>
		<?php
	}
	...
	function row_title($topic = array()){
		...
		$url = r_('index.php?ext=chronoforums&cont=posts&f='.$topic['Topic']['forum_id'].'&t='.$topic['Topic']['id'].'&alias='.$topic['Topic']['alias']);
		if(!empty($searched)){
	...
	function topic_title($topic = array()){
		?>
		<a class="cfu-title" href="<?php echo r_('index.php?ext=chronoforums&cont=posts&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id'].'&alias='.$topic['Topic']['alias']); ?>"><?php echo strip_tags($topic['Topic']['title']); ?></a>
		<?php
	}

	function selector($topic = array()){
		echo $this->view->Html->input('topics_ids['.$topic['Topic']['id'].']', array('type' => 'checkbox', 'class' => 'selector_checkbox', 'value' => 1));
	}

	function topic_category($topic = array()){
		?>
		<a class="cfu-category-title" href="<?php echo r_('index.php?ext=chronoforums&c='.$topic['Category']['id']); ?>"><?php echo $topic['Category']['title']; ?></a>
		<?php
	}

	function topic_forum($topic = array()){
		?>
		<a class="cfu-forum-title" href="<?php echo r_('index.php?ext=chronoforums&cont=forums&f='.$topic['Forum']['id'].'&alias='.$topic['Forum']['alias']); ?>"><?php echo $topic['Forum']['title']; ?></a>
		<?php
	}
	...
	function status($topic = array()){
		...
		<?php /*if(!empty($topic['Topic']['created'])): ?>
		<span class="label label-default gcoreTooltip" style="background-color:#424242;" title="<?php echo $this->view->Output->date_time($topic['Topic']['created']); ?>"><i class="fa fa-calendar fa-lg"></i>&nbsp;<?php echo \GCore\Libs\Date::ago(strtotime($topic['Topic']['created'])); ?></span>
		<?php endif;*/ ?>
		...
		<?php if(!empty($topic['Topic']['post_count']) AND $topic['Topic']['post_count'] >= $fparams->get('topic_hot_limit', 5) AND $topic['Topic']['post_count']/$days >= $fparams->get('topic_hot_limit', 5)): ?>
		...
		<span class="label label-default" style="background-color:#585858;"><i class="fa fa-desktop fa-lg gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_VIEWS'); ?>"></i>&nbsp;<?php echo $topic['Topic']['hits']; ?></span>
		<?php endif; ?>
		...
		<div class="btn-group btn-group-xs cfu-mini-pager">
			...
			<?php for($i = $list_start; $i <= ceil((int)$topic['Topic']['post_count']/(int)$fparams->get('posts_limit', 10)); $i++): ?>
				<a class="btn btn-default" href="<?php echo r_('index.php?ext=chronoforums&cont=posts&f='.$topic['Topic']['forum_id'].'&t='.$topic['Topic']['id'].'&alias='.$topic['Topic']['alias'].'&page='.$i); ?>"><?php echo $i; ?></a>
			<?php endfor; ?>
		</div>
		<?php endif; ?>
		
		<?php /*if((bool)$fparams->get('enable_topic_quick_preview', 1) === true): ?>
		<a title="" class="btn btn-default btn-xs" href="<?php echo r_('index.php?ext=chronoforums&cont=posts&tvout=ajax&act=topic_preview&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id']); ?>" onclick="topic_quick_review(this); return false;"><i class="fa fa-external-link"></i></a>
		<?php endif;*/ ?>
		
		<?php
	}
	
	function edit_link($topic = array()){
		$fparams = $this->view->vars['fparams'];
		if(!empty($topic['Topic'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_EDIT_TOPIC'); ?>" class="btn btn-default gcoreTooltip text-primary" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=edit&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-pencil fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function move_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo l_('CHRONOFORUMS_MOVE_TOPIC'); ?>" class="btn btn-default gcoreTooltip text-primary" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=move&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-external-link-square fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function lock_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			...
			$icon = !empty($topic['Topic']['locked']) ? 'lock' : 'unlock';
			$tip = !empty($topic['Topic']['locked']) ? l_('CHRONOFORUMS_UNLOCK_TOPIC') : l_('CHRONOFORUMS_LOCK_TOPIC');
			$class = !empty($topic['Topic']['locked']) ? 'text-success btn-warning active' : 'text-danger btn-default';
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo $tip; ?>" class="btn gcoreTooltip <?php echo $class; ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=".$act."&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-<?php echo $icon; ?> fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function publish_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			...
			$icon = !empty($topic['Topic']['published']) ? 'check-circle' : 'check-circle';
			$tip = !empty($topic['Topic']['published']) ? l_('CHRONOFORUMS_UNPUBLISH_TOPIC') : l_('CHRONOFORUMS_PUBLISH_TOPIC');
			$class = !empty($topic['Topic']['published']) ? 'text-danger btn-success active' : 'text-success btn-default';
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo $tip; ?>" class="btn gcoreTooltip <?php echo $class; ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=".$act."&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-<?php echo $icon; ?> fa-lg"></i></a>
			<?php endif; ?>
	...
	function announce_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			...
			$class = !empty($topic['Topic']['announce']) ? 'text-danger btn-primary active' : 'text-success btn-default';
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo $tip; ?>" class="btn gcoreTooltip <?php echo $class; ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=".$act."&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-<?php echo $icon; ?> fa-lg"></i></a>
			<?php endif; ?>
	...
	function sticky_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			...
			$class = !empty($topic['Topic']['sticky']) ? 'text-danger btn-primary active' : 'text-success btn-default';
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo $tip; ?>" class="btn gcoreTooltip <?php echo $class; ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=".$act."&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-<?php echo $icon; ?> fa-lg"></i></a>
			<?php endif; ?>
	...
	function delete_link($topic = array()){
		...
		if(!empty($topic['Topic'])):
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics', $topic['Topic']['user_id'])): ?>
			<a title="<?php echo l_('CHRONOFORUMS_DELETE_TOPIC'); ?>" class="btn gcoreTooltip text-danger btn-danger" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=delete&t=".$topic['Topic']['id']."&f=".$topic['Topic']['forum_id']); ?>"><i class="fa fa-trash-o fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function delete_author_link($topic = array()){
		...
		if(!empty($topic['Topic']) AND !empty($topic['Topic']['user_id'])):
			...
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') AND (bool)$fparams->get('allow_author_delete', 1) === true): ?>
				<a title="<?php echo l_('CHRONOFORUMS_DELETE_AUTHOR'); ?>" class="btn gcoreTooltip text-danger btn-danger" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=delete_author&u=".$topic['Topic']['user_id']."&f=".$topic['Topic']['forum_id']."&t=".$topic['Topic']['id']); ?>"><i class="fa fa fa-user fa-lg text-default"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
		...
		$doc->addJsCode('
			...
			function load_more_posts(topic_id){
				if(!posts_end_reached && !posts_loader_running){
					jQuery.ajax({
						"type" : "POST",
						"url" : "'.r_('index.php?ext=chronoforums&cont=posts&tvout=ajax&posts_loader=1').'",
		...
		$doc->addJsCode('
			...
			function load_more_posts(topic_id){
				if(!posts_end_reached && !topic_preview_running){
					jQuery.ajax({
						"type" : "POST",
						"url" : "'.r_('index.php?ext=chronoforums&cont=posts&tvout=ajax&act=topic_preview').'",
