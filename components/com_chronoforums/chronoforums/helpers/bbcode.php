...
class Bbcode {
	...
	function parse($text) {
		...
		if($fparams->get('update_emails_display', 1)){
			//fix emails
		...
		if(!empty($hilit)){
			...
			$text = preg_replace('/('.implode('|', $hilits).')/ims', '<span class="cfu-hilit">${1}</span>', $text);
		}
		...
		$out = array(
			...
			'<a class="cfu-postlink" target="_blank" rel="nofollow" href="\2\3">\2\3</a>',
			'<a class="cfu-postlink" target="_blank" rel="nofollow" href="http://\2">\2</a>',
