...
class PostTasks {
	...
	function votes_label($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			?>
			<span class="label <?php echo $class; ?><?php if(!empty($post['Vote'][0]['voters'])): ?> gcoreTooltip<?php endif; ?>" title="<?php echo !empty($post['Vote'][0]['voters']) ? $post['Vote'][0]['voters'] : ''; ?>"><i class="fa <?php echo $icon_class; ?> fa-lg"></i>&nbsp;<?php echo $post['Vote'][0]['votes_sum']; ?></span>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function vote_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_VOTE_DOWN'); ?>" class="btn btn-default gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=vote&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']."&val=-1"); ?>"><i class="fa fa-thumbs-down fa-lg text-danger"></i></a>
			<?php endif; ?>
			<a title="<?php echo l_('CHRONOFORUMS_VOTE_UP'); ?>" class="btn btn-default gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=vote&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']."&val=1"); ?>"><i class="fa fa-thumbs-up fa-lg text-success"></i></a>
			</span>
			<?php endif; ?>
			<?php
		endif;
	...
	function select_answer_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_SELECT_ANSWER'); ?>" class="btn btn-success gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=answer&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-check fa-lg"></i></a>
			<?php endif; ?>
			<?php if(!empty($topic['Answer']['topic_id']) AND ($topic['Answer']['post_id'] == $post['Post']['id']) AND \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics')): ?>
			<a title="<?php echo l_('CHRONOFORUMS_UNSELECT_ANSWER'); ?>" class="btn btn-success active gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=unanswer&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-check fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function quote_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_REPLY_QUOTE'); ?>" class="btn btn-primary gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=reply_quote&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-quote-right fa-lg"></i><?php //echo l_('CHRONOFORUMS_QUOTE'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function report_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_REPORT'); ?>" class="btn btn-warning gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=report&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-warning fa-lg"></i><?php //echo l_('CHRONOFORUMS_REPORT'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function publish_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_PUBLISH'); ?>" class="btn btn-default gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=publish&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-toggle-off fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function unpublish_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics')): ?>
			<a title="<?php echo l_('CHRONOFORUMS_UNPUBLISH'); ?>" class="btn btn-success gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=unpublish&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-toggle-on fa-lg"></i></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function edit_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_EDIT'); ?>" class="btn btn-default gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=edit&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-pencil fa-lg"></i><?php //echo l_('CHRONOFORUMS_EDIT'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	
	function split_link($post = array(), $topic = array()){
		$fparams = $this->view->vars['fparams'];
		if(!empty($post['Post']) AND $post['Post']['created'] != $topic['Topic']['created']):
			?>
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics')): ?>
			<a title="<?php echo l_('CHRONOFORUMS_SPLIT_TOPIC'); ?>" class="btn btn-default gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=split&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-cut fa-lg"></i><?php //echo l_('CHRONOFORUMS_EDIT'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function delete_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post'])):
			...
			<a title="<?php echo l_('CHRONOFORUMS_DELETE'); ?>" class="btn btn-danger gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=posts&act=delete&p=".$post['Post']['id']."&t=".$post['Post']['topic_id']."&f=".$post['Post']['forum_id']); ?>"><i class="fa fa-trash-o fa-lg"></i><?php //echo l_('CHRONOFORUMS_DELETE'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function delete_author_link($post = array(), $topic = array()){
		...
		if(!empty($post['Post']) AND !empty($post['PostAuthor']['id'])):
			...
			<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') AND (bool)$fparams->get('allow_author_delete', 1) === true): ?>
				<a title="<?php echo l_('CHRONOFORUMS_DELETE_AUTHOR'); ?>" class="btn btn-danger gcoreTooltip" href="<?php echo r_("index.php?ext=chronoforums&cont=topics&act=delete_author&p=".$post['Post']['id']."&u=".$post['PostAuthor']['id']."&f=".$post['Post']['forum_id']."&t=".$post['Post']['topic_id']); ?>"><i class="fa fa fa-user fa-lg text-default"></i><?php //echo l_('CHRONOFORUMS_DELETE_AUTHOR'); ?></a>
			<?php endif; ?>
			<?php
		endif;
	}
	...
	function content($post = array()){
		...
		if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'list_attachments')){
			...
			if(!empty($post['Attachment'])){
				...
				foreach($post['Attachment'] as $k => $attach){
					...
					$post['Attachment'][$k]['link'] = r_('index.php?ext=chronoforums&cont=attachments&act=download&file_id='.$post['Attachment'][$k]['id']);
					...
					if(in_array(strtolower($ext), $inline_extensions) AND \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'download_attachments')){
						...
						$post['Attachment'][$k]['inline_output'] = '<div class="cfu-inline-attachment"><dl class="cfu-file"><dt><i title="" class="fa fa-fw fa-paperclip" data-original-title="Attachment"></i><a class="cfu-link" href="'.$post['Attachment'][$k]['link'].'">'.$post['Attachment'][$k]['filename'].'</a></dt><dd><em>'.$post['Attachment'][$k]['comment'].'</em></dd><dd>'.$post['Attachment'][$k]['size'].' '.l_('CHRONOFORUMS_DOWNLOADED').' '.$post['Attachment'][$k]['downloads'].' '.l_('CHRONOFORUMS_TIMES').'.</dd></dl></div>';
						$post['Attachment'][$k]['output'] = '<dl class="cfu-file"><dt><i title="" class="fa fa-fw fa-paperclip" data-original-title="Attachment"></i><a class="cfu-link" href="'.$post['Attachment'][$k]['link'].'">'.$post['Attachment'][$k]['filename'].'</a></dt><dd><em>'.$post['Attachment'][$k]['comment'].'</em></dd><dd>'.$post['Attachment'][$k]['size'].' '.l_('CHRONOFORUMS_DOWNLOADED').' '.$post['Attachment'][$k]['downloads'].' '.l_('CHRONOFORUMS_TIMES').'.</dd></dl>';
					}
				}
			}
		}
		...
		if($fparams->get('enable_private_data', 0)){
			$content = preg_replace_callback('/\[private\](.*?)\[\/private\]/msi', function($s) use ($post){
				$private_area = '
				<div class="panel panel-_TYPE_ cfu-private-content-area">
					<div class="panel-heading"><i class="fa fa-lock fa-fw fa-lg"></i>&nbsp;'.l_('CHRONOFORUMS_PRIVATE').'</div>
					<div class="panel-body">_CONTENT_</div>
				</div>';
				if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'view_private', $post['PostAuthor']['id'])){
					return str_replace(array('_TYPE_', '_CONTENT_'), array('info', $s[1]), $private_area);
				}else{
					return str_replace(array('_TYPE_', '_CONTENT_'), array('danger', l_('CHRONOFORUMS_PRIVATE_CONTENT_LOCKED')), $private_area);
				}
			}, $content);
		}
	...
	function search_form($topic = array()){
		...
		?>
		<form action="<?php echo r_('index.php?ext=chronoforums&cont=posts&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id']); ?>" method="post" name="searchform">
		...
		<?php if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'view_reports')): ?>
			<?php if(!empty($post['Report'])): ?>
				<div class="alert alert-danger cfu-post-report">
					...
					<br />
					<a href="<?php echo r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Post']['topic_id'].'&act=unreport&report_id='.$report['id']); ?>" class="btn btn-danger btn-xs"><?php echo l_('CHRONOFORUMS_DELETE'); ?></a>
					<?php endif; ?>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
	...
	function title($post = array(), $topic = array()){
		...
		?>
		<a class="cfu-post-title" href="<?php echo r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Post']['topic_id'].'&alias='.$alias.'#p'.$post['Post']['id']); ?>"><?php echo strip_tags($post['Post']['subject']); ?></a>
		<?php
	}
}