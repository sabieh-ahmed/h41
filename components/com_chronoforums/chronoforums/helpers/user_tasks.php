...
class UserTasks {
	...
	function get_ranks_titles($user = array()){
		if(empty($user['Profile']['ranks'])){
			return array();
		}
		$fparams = $this->view->vars['fparams'];
		$ranks = !empty($this->view->vars['ranks']) ? $this->view->vars['ranks'] : array();
		$ranks_titles = \GCore\Libs\Arr::getVal($ranks, array('[n]', 'Rank', 'title'));
		$ids = \GCore\Libs\Arr::getVal($ranks, array('[n]', 'Rank', 'id'));
		$ranks_list = array_combine($ids, $ranks_titles);
		$user_ranks = array_flip($user['Profile']['ranks']);
		$outputs = array_intersect_key($ranks_list, $user_ranks);
		return $outputs;
	}
	...
	function username($user = array(), $avatar = true){
		...
		<?php if(!empty($user['User']['id'])): ?>
			...
			?>
			<span class="cfu-username-box" data-ajax="<?php echo r_('index.php?ext=chronoforums&cont=profiles&act=mini&u='.$user['User']['id'].'&tvout=ajax'); ?>">
	...
	function avatar($user = array(), $params = array()){
		...
		//$img_params[':data-ajax'] = r_('index.php?ext=chronoforums&cont=profiles&act=mini&tvout=ajax&img='.$user['Profile']['params']['avatar']);
		return \GCore\Helpers\Html::url(\GCore\Helpers\Html::image(r_('index.php?ext=chronoforums&cont=profiles&act=avatar&img='.$user['Profile']['params']['avatar']), $img_params), r_(\GCore\Libs\Str::replacer($fparams->get('username_link_path', ''), array('id' => $user['User']['id']))));
	}
	
	function signature($user = array()){
		$fparams = $this->view->vars['fparams'];
		return $this->view->Bbcode->parse($user['Profile']['params']['signature']);
	}
