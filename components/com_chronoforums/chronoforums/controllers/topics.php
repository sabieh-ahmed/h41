...
class Topics extends \GCore\Extensions\Chronoforums\Chronoforums {
	...
	function index(){
		$this->redirect(r_('index.php?ext=chronoforums&cont=forums&f='.\GCore\Libs\Request::data('f', 0)));
	}
	...
	function add($quote = false){
		...
		/*if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('make_topics', $forum['Forum'])){
			//trigger permissions errors
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=topics&act=add&f='.$fid));
			
			$session->setFlash('error', l_('CHRONOFORUMS_PERMISSIONS_ERROR'));
			$this->redirect(r_('index.php?ext=chronoforums&cont=forums&f='.$fid));
		}*/
		...
		if(!empty($fid)){
			...
			if(!empty($forum)){
				/*if((bool)$this->fparams->get('forum_permissions', 0) === true AND !\GCore\Libs\Authorize::check_rules($forum['Forum']['rules']['make_topics'])){
					...
					$this->redirect(r_('index.php?ext=chronoforums&cont=forums&f='.$forum['Forum']['id']));
				}*/
				if(!empty($this->data['Post'])){
					if(!empty($this->data['upload'])){
						...
						if($result !== false){
							...
							//send email
							$this->data['Topic']['title_link'] = \GCore\Helpers\Html::url($this->data['Topic']['title'], r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$this->data['Post']['topic_id'].'#p'.$this->Post->id, false, true));
							$this->data['Forum'] = $forum['Forum'];
							...
							if(((int)$this->fparams->get('send_email_on_topic', 0) === 1 OR ((int)$this->fparams->get('send_email_on_topic', 0) === 2 AND empty($this->data['Topic']['published']))) AND $this->fparams->get('topics_notify_groups', array())){
								//if(\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $forum['Forum'])){
							...
							if($this->fparams->get('enable_auto_reply', 0)){
								$url = r_('index.php?ext=chronoforums&cont=posts&act=auto_reply&t='.$this->Topic->id, false, true);
							...
							if(!empty($this->data['Topic']['published'])){
								$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$this->Topic->id.'#p'.$this->Post->id));
							}else{
		...
		if($this->fparams->get('board_display', 'default') == 'discussions' OR empty($fid)){
			$forums_list = array();
			$cats = $this->Forum->find('all', array('cache' => true, 'fields' => array('Category.id', 'Category.title', 'Category.published', 'Forum.*'), 'contain' => array('Category', 'Forum')));
			foreach($cats as $cat){
				if(!empty($cat['Category']['published']) AND !empty($cat['Forum']['published'])){
					if(\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $cat['Forum']) AND \GCore\Extensions\Chronoforums\Chronoforums::_check_perms('make_topics', $cat['Forum'])){
	...
	function edit(){
		...
		/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') !== true){
			//trigger permissions errors
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=topics&act=edit&t='.$tid));
		...
		if(!empty($tid)){
			...
			if(!empty($this->data['Post'])){
				if(!empty($this->data['cancel_post'])){
					...
					$this->Topic->id = $tid;
					$topic['Topic']['params']['fields'] = $this->data['topic_info'];
					$result = $this->Topic->updateAll(array('title' => $this->data['Post']['subject'], 'alias' => '', 'params' => $topic['Topic']['params']), array('id' => $tid));
					if($result !== false){
						$session->setFlash('success', l_('CHRONOFORUMS_TOPIC_UPDATE_SUCCESS'));
						$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$tid.'&f='.$this->data['f']));
					}else{							
						$session->setFlash('error', l_('CHRONOFORUMS_SAVE_ERROR'));
					}
				}
			}else{
				$this->data['Post']['subject'] = $topic['Topic']['title'];
				if(!empty($topic['Topic']['params']['fields'])){
	...
	function split(){
		...
		/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') !== true){
			//trigger permissions errors
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=topics&act=split&t='.$tid.'&p='.$pid));
		...
		if(!empty($tid) AND !empty($pid)){
			$topic = $this->Topic->find('first', array('conditions' => array('id' => $tid), 'contain' => array()));
			$post = $this->Post->find('first', array('conditions' => array('id' => $pid), 'contain' => array()));
			//pr($post);
			if(!empty($this->data['Post'])){
				if(!empty($this->data['cancel_post'])){
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$tid));
					...
					$topic2 = $topic;
					$topic2['Topic']['params']['fields'] = $this->data['topic_info'];
					$topic2['Topic']['title'] = $this->data['Post']['subject'];
					$topic2['Topic']['alias'] = '';
					$topic2['Topic']['hits'] = 0;
					$topic2['Topic']['created'] = $post['Post']['created'];
					$topic2['Topic']['user_id'] = $post['Post']['user_id'];//$user['id'];
					$topic2['Topic']['params']['uid'] = \GCore\Libs\Str::uuid();
					unset($topic2['Topic']['id']);
					unset($topic2['Topic']['created']);
					unset($topic2['Topic']['modified']);
					unset($topic2['Topic']['reported']);
					unset($topic2['Topic']['has_attachments']);
					unset($topic2['Topic']['announce']);
					unset($topic2['Topic']['sticky']);
					//create the topic
					$result_1 = $this->Topic->save($topic2);
					//assign the posts to the new topic
					$result_2 = $this->Post->updateAll(array('topic_id' => $this->Topic->id), array('id >=' => $pid, 'topic_id' => $tid));
					//fix attachments, votes, reports topic id
					$this->Post->Attachment->updateAll(array('topic_id' => $this->Topic->id), array('post_id >=' => $pid, 'topic_id' => $tid));
					$this->Post->Report->updateAll(array('topic_id' => $this->Topic->id), array('post_id >=' => $pid, 'topic_id' => $tid));
					$this->Vote->updateAll(array('topic_id' => $this->Topic->id), array('post_id >=' => $pid, 'topic_id' => $tid));
					
					$tid2 = $this->Topic->id;
					//fix the attachment and reported flags for both topics
					//new topic
					$topic_has_reports = $this->Post->Report->field('id', array('Report.topic_id' => $tid2));
					$topic_has_attachments = $this->Post->Attachment->field('id', array('Attachment.topic_id' => $tid2));
					...
					//old topic
					$topic_has_reports = $this->Post->Report->field('id', array('Report.topic_id' => $tid));
					$topic_has_attachments = $this->Post->Attachment->field('id', array('Attachment.topic_id' => $tid));
					...
					//old topic last post
					$last_post = $this->Post->find('first', array('conditions' => array('topic_id' => $tid), 'fields' => array('id'), 'order' => 'Post.created DESC'));
					if(!empty($last_post)){
						$this->Topic->saveField('last_post', $last_post['Post']['id']);
					}
					//save topic subscriptions
					$subs = \GCore\Admin\Extensions\Chronoforums\Models\Subscribed::getInstance()->find('all', array('conditions' => array('topic_id' => $tid)));
					if(!empty($subs)){
						foreach($subs as $k => $sub){
							$subs[$k]['Subscribed']['topic_id'] = $tid2;
						}
						\GCore\Admin\Extensions\Chronoforums\Models\Subscribed::getInstance()->saveAll($subs);
					}
					
					//go to new topic
					if($result_1 !== false AND $result_2 !== false){
						$session->setFlash('success', l_('CHRONOFORUMS_TOPIC_SPLIT_SUCCESS'));
						$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$tid2.'&f='.$this->data['f']));
					}else{							
						$session->setFlash('error', l_('CHRONOFORUMS_SAVE_ERROR'));
					}
				}
	...
	function move(){
		...
		$this->Topic = \GCore\Admin\Extensions\Chronoforums\Models\Topic::getInstance(array('allowed_models' => array('Topic', 'Forum')));
		$this->Post = \GCore\Admin\Extensions\Chronoforums\Models\Post::getInstance(array('allowed_models' => array('Post', 'Forum')));
		$this->Forum = \GCore\Admin\Extensions\Chronoforums\Models\Forum::getInstance(array('allowed_models' => array('Forum', 'Category')));
	...
	function delete(){
		$this->Topic = \GCore\Admin\Extensions\Chronoforums\Models\Topic::getInstance(array('allowed_models' => array('Topic', 'Forum')));
		$this->Post = \GCore\Admin\Extensions\Chronoforums\Models\Post::getInstance(array('allowed_models' => array('Post', 'Attachment', 'Report', 'Forum')));
		...
		if(!empty($this->data['confirm_delete'])){
			...
			$topics = $this->Topic->find('all', array('conditions' => array('id' => $tids)));
			$this->set('topics', $topics);
		}
	}
	...
	function delete_author(){
		$session = \GCore\Libs\Base::getSession();
		$user = \GCore\Libs\Base::getUser();
		if((bool)$this->fparams->get('allow_author_delete', 1) !== true){
			$session->setFlash('error', l_('CHRONOFORUMS_PERMISSIONS_ERROR'));
			$this->redirect(r_('index.php?ext=chronoforums'));
		}
		$this->Topic = \GCore\Admin\Extensions\Chronoforums\Models\Topic::getInstance(array('allowed_models' => array('Topic', 'Forum')));
	...
	function subscribe(){
		...
		/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_posts') !== true){
			//trigger permissions errors
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=topics&act=subscribe&t='.$tid));
	...
	function unsubscribe(){
		...
		/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_posts') !== true){
			//trigger permissions errors
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=topics&act=unsubscribe&t='.$tid));
