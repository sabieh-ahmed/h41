...
class Posts extends \GCore\Extensions\Chronoforums\Chronoforums {
	...
	function index(){
		...
		if(!empty($pid)){
			...
			*/
			...
			$post = $this->Post->find('first', array('fields' => array('topic_id', 'created'), 'conditions' => array('id' => $pid)));
			
			if(!empty($post)){
				if(empty($tid)){
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$post['Post']['topic_id'].'&p='.$pid.'#p'.$pid));
				}
				
				$post_date = $post['Post']['created'];
				...
				if(!empty($tid)){
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$tid));
				}
			}
		}
		if(!empty($tid)){
			...
			if($this->fparams->get('topic_status_author', 0)){
				...
				$this->Topic->contain[] = 'TopicAuthor';
			...
			//get topic data
			$topic = $this->Topic->find('first', array('cache' => true, 'conditions' => array('id' => $tid), 'contain' => array('Forum'), 'fields' => array('Topic.*', 'Answer.*', 'Favorite.*', 'Featured.*', 'TopicAuthor.*', 'TopicAuthorProfile.*', 'Forum.*')));
			//pr($topic);
			if(empty($topic)){
			...
			//check permissions
			/*if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $topic['Forum'])){
				//trigger permissions errors
				\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&f='.$fid.'&t='.$tid));
				
				$session->setFlash('error', l_('CHRONOFORUMS_FORUM_ACCESS_DENIED'));
				\GCore\Libs\Env::e404();
				return false;
			}*/
			...
			//check permissions
			/*if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('read_topics', $topic['Forum'], $topic['Topic']['user_id'])){
				//trigger permissions errors
				\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&f='.$fid.'&t='.$tid));
			...
			if($this->fparams->get('enable_votes', 0) == 'post'){
				...
				if(0){
					$this->Post->Vote->bindModels('belongsTo', array(
						'Voter' => array(
							'className' => '\GCore\Admin\Models\User',
							'fields' => array(':GROUP_CONCAT(Voter.username)' => 'voters'),
						),
					));
					$this->Post->contain[] = 'Voter';
				}
			...
			if((bool)$this->fparams->get('enable_topic_views', 1) === true){
				$this->Topic->id = $tid;
				$this->Topic->updateField('hits', '+ 1', array('recursive' => -1));
			}
			...
			if(!empty($this->helpers['\GCore\Helpers\Paginator']['total']) AND $this->helpers['\GCore\Helpers\Paginator']['total'] != $topic['Topic']['post_count']){
				...
				//not needed after fixing the count cache
			...
			if(!empty($posts)){
				//set meta data
				\GCore\Libs\Event::trigger('on_set_meta_info', $posts[0]['Post']['text']);
			}
		...
		if(!empty($tid)){
			...
			if(!empty($topic)){
				if(!empty($topic['Topic']['locked'])){
					...
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&&t='.$topic['Topic']['id'].'&f='.$topic['Forum']['id']));
				}
				...
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'make_posts') !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=reply&t='.$topic['Topic']['id'].'&f='.$topic['Forum']['id']));
				...
				/*if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('make_posts', $topic['Forum'], $topic['Topic']['user_id'])){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=reply&t='.$topic['Topic']['id'].'&f='.$topic['Forum']['id']));
				...
				if(!empty($this->data['Post'])){
					if(!empty($this->data['upload'])){
						$this->_upload_file();
						if(!empty($this->data['tvout']) AND $this->data['tvout'] == 'ajax'){
							//ajax upload
							//echo json_encode($this->data['Attachment'][0]);
							//$this->view = false;
							$this->view = \GCore\C::ext_path('chronoforums', 'front').'views'.DS.'ajax_attachments.php';
							return;
						}
						...
						if((bool)$this->fparams->get('new_posts_published', 1) === true OR \GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics')){
							...
							if((int)$this->fparams->get('auto_approval_threshold', 0) AND !empty($user['id'])){
							...
							if((int)$this->fparams->get('non_approved_threshold', 0) AND !empty($user['id'])){
						...
						$this->data['Topic']['title_link'] = \GCore\Helpers\Html::url($this->data['Topic']['title'], r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$tid.'#p'.$this->Post->id, false, true));
						...
						if($result !== false){
							...
							//$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$tid.'#p'.$this->Post->id));
							if(!empty($this->data['Post']['published'])){
								$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$tid.'#p'.$this->Post->id));
							}else{
								$session->setFlash('success', l_('CHRONOFORUMS_POST_POSTED_NOT_PUBLISHED'));
								$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$tid));
							}
						}else{							
					...
					if($quote){
						...
						$this->data['Post']['text'] = '[quote="'.$post['PostAuthor'][$this->fparams->get('display_name', 'username')].'"]'.$post['Post']['text'].'[/quote]';
					}
				}
			}
		}else{
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'edit_posts', $post['PostAuthor']['id']) !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=edit&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
					
					$session->setFlash('error', l_('CHRONOFORUMS_PERMISSIONS_ERROR'));
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
				}*/
				if(!empty($this->data['Post'])){
					if(!empty($this->data['upload'])){
						$this->_upload_file();
						...
						$this->data['Topic']['title_link'] = \GCore\Helpers\Html::url($post['Topic']['title'], r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$post['Topic']['id'].'#p'.$this->Post->id, false, true));
						...
						if($result !== false){
							...
							$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$post['Topic']['id'].'#p'.$this->Post->id));
						}else{							
							$session->setFlash('error', l_('CHRONOFORUMS_SAVE_ERROR'));
						}
					}
	...
	function delete(){
		$this->Topic = \GCore\Admin\Extensions\Chronoforums\Models\Topic::getInstance(array('allowed_models' => array('Topic', 'Forum')));
		$this->Post = \GCore\Admin\Extensions\Chronoforums\Models\Post::getInstance(array('allowed_models' => array('Post', 'Topic', 'Forum', 'PostAuthor', 'Attachment', 'Report', 'ReportAuthor')));
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'delete_posts', $post['PostAuthor']['id']) !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=delete&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
					...
					if($result !== false){
						...
						/*if($pid == $post['Topic']['last_post']){
							...
							if(!empty($last_post)){
								...
								if($topic_deleted){
									$this->redirect(r_('index.php?ext=chronoforums&cont=forums&f='.$post['Post']['forum_id']));
								}
							}
						}*/
						//check if this was the only post
						$last_post = $this->Post->find('first', array('conditions' => array('topic_id' => $post['Topic']['id']), 'fields' => array('id'), 'order' => 'Post.created DESC'));
						if(empty($last_post)){
							//topic had this post only and it was the last one, delete topic
							$topic_deleted = $this->Topic->delete($post['Topic']['id']);
						...
						$this->redirect(r_('index.php?ext=chronoforums&cont=posts&t='.$post['Topic']['id']));
					}else{							
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'report_posts') !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=report&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
				...
				if(!empty($this->data['confirm_report'])){
					...
					if($result !== false){
						...
						if((bool)$this->fparams->get('send_email_on_report', 1) === true){
							$this->data['url'] = r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id'], false, true);
	...
	function publish(){
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=publish&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
	...
	function unpublish(){
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=unpublish&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'select_answers', $post['Topic']['user_id']) !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=answer&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
	...
	function unanswer(){
		...
		if(!empty($pid)){
			...
			if(!empty($post)){
				/*if(\GCore\Libs\Authorize::authorized('\GCore\Extensions\Chronoforums\Chronoforums', 'modify_topics') !== true){
					//trigger permissions errors
					\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=posts&act=unanswer&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
					
					$session->setFlash('error', l_('CHRONOFORUMS_PERMISSIONS_ERROR'));
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
				}*/
				$result = \GCore\Admin\Extensions\Chronoforums\Models\Answer::getInstance()->delete(array('post_id' => $pid, 'topic_id' => $post['Topic']['id']));
				
				if($result !== false){
					$session->setFlash('success', l_('CHRONOFORUMS_ANSWER_DELETED'));
					$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$post['Post']['id'].'&t='.$post['Topic']['id'].'#p'.$post['Post']['id']));
				}else{							
					$session->setFlash('error', l_('CHRONOFORUMS_SAVE_ERROR'));
				}
	...
	function vote(){
		...
		if($this->fparams->get('enable_votes', 0) == 'post' AND !empty($pid) AND !empty($this->data['val']) AND in_array((int)$this->data['val'], $valid_votes)){
			...
			$this->redirect(r_('index.php?ext=chronoforums&cont=posts&p='.$pid.'&t='.$tid.'#p'.$pid));
		}
	}
	
	function email_reply(){
		$this->Topic = \GCore\Admin\Extensions\Chronoforums\Models\Topic::getInstance(array('allowed_models' => array('Topic', 'Forum')));
		$this->Post = \GCore\Admin\Extensions\Chronoforums\Models\Post::getInstance(array('allowed_models' => array('Topic', 'Forum', 'Post', 'PostAuthor', 'Attachment', 'Report', 'ReportAuthor')));
		...
		foreach($msgs as $k => $msg){
			if(preg_match('/\[TUID#(.*?)\]/', imap_utf8($msg->subject), $matches)){
				if(!empty($matches[1])){
					$tuid = $matches[1];
					...
					if(!empty($topic) AND !empty($user)){
						...
						if(!empty($subscribed)){
							...
							if(!empty($this->data['Post'])){
								...
								$this->data['Topic']['title_link'] = \GCore\Helpers\Html::url($this->data['Topic']['title'], r_(\GCore\Libs\Url::root(true).'index.php?ext=chronoforums&cont=posts&p='.$this->Post->id.'&t='.$tid.'#p'.$this->Post->id));
