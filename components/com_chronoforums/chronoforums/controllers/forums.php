...
class Forums extends \GCore\Extensions\Chronoforums\Chronoforums {
	...
	function index(){
		...
		if(empty($fid)){
			...
			if(empty($skeywords)){
				if($this->fparams->get('board_display', 'default') == 'discussions'){
					...
					if(!empty($list) AND in_array($list, array('noreplies', 'active', 'new', 'user', 'favorites', 'featured', 'unpublished', 'tagged'))){
						$global_listing = true;
		...
		$this->Forum = \GCore\Admin\Extensions\Chronoforums\Models\Forum::getInstance(array('allowed_models' => array('Category', 'Forum')));
		...
		if($global_listing === false){
			$forum = $this->Forum->find('first', array('cache' => true, 'conditions' => array('id' => $fid), 'contain' => array('Forum'), 'fields' => array('Forum.id', 'Forum.title', 'Forum.alias', 'Forum.published', 'Forum.description', 'Forum.rules', 'Forum.params')));
			...
			if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $forum['Forum'])){
				...
				\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=forums&f='.$fid));
		...
		if($global_listing === true){
			...
			//check forums access permissions
			$forums_list = array();
			$cats = $this->Forum->find('all', array('cache' => true, 'contain' => array('Category', 'Forum')));
			foreach($cats as $cat){
				if(!empty($cat['Category']['published']) AND !empty($cat['Forum']['published'])){
					if(\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $cat['Forum'])){
						$forums_list[] = $cat['Forum']['id'];
					}
				}
			}
			$this->Topic->conditions['forum_id'] = $forums_list;
		}
		...
		if(\GCore\Libs\Request::data('list') == 'tagged'){
			$this->Tag = \GCore\Admin\Extensions\Chronoforums\Models\Tag::getInstance();
			$tag_id = $this->Tag->field('id', array('Tag.title' => \GCore\Libs\Request::data('tag')));
			...
			if(!empty($tag_id)){
				$this->Topic->bindModels('hasOne', array(
					'Tagged' => array(
						...
						'type' => 'inner',
						'group' => 'topic_id',
						'conditions' => array('Tagged.tag_id' => $tag_id),
					),
				));
	...
	function subscriptions(){
		$time_difference = (int)$this->fparams->get('forums_subscriptions_frequency', 24) * 60 * 60;
		$last_sent = $this->fparams->get('forums_subscriptions_last_sent', 0);
		if($last_sent + $time_difference < time()){
			if($last_sent > 0){
				//find the users to send the notifications to
				$this->Post = \GCore\Admin\Extensions\Chronoforums\Models\Post::getInstance(array('allowed_models' => array('Post', 'Topic', 'PostAuthor', 'GroupUser')));
				$emails_list = $this->Post->PostAuthor->find('list', array('fields' => array('email', 'name', 'id'), 'conditions' => array('GroupUser.group_id' => $this->fparams->get('forums_subscriptions_groups', array()))));
				$mods = !empty($emails_list) ? array_keys($emails_list) : array();
				foreach($mods as $mod){
					$this->_sendEmail('forums_subscriptions', array($mod => $mod));
				}
			}
			//update the last sent/checked time
			parent::_settings('chronoforums');
			$this->data['Chronoforums']['forums_subscriptions_last_sent'] = time();
