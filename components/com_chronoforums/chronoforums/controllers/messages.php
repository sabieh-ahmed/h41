...
class Messages extends \GCore\Extensions\Chronoforums\Chronoforums {
	...
	function inbox(){
		...
		$this->_load();
		//$this->Message->group = array('MessageRecipient.discussion_id');
	...
	function delete(){
		...
		$this->redirect(r_('index.php?ext=chronoforums&cont=messages'));
	}
	...
	function username_lookup(){
		...
		if(!empty($this->data['username_q'])){
			$search_filter = $this->fparams->get('pm_recipients_filters', 'username');
			if($search_filter == 'username'){
				$like = array('User.username LIKE' => $this->data['username_q'].'%');
				$fields = array('User.id', 'User.username');
			}else if($search_filter == 'name'){
				$like = array('User.name LIKE' => $this->data['username_q'].'%');
				$fields = array('User.id', 'User.name');
			}else if($search_filter == 'both'){
				$like = array('OR' => array('User.name LIKE' => $this->data['username_q'].'%', 'User.username LIKE' => $this->data['username_q'].'%'));
				$fields = array('User.id', ':CONCAT(User.username, " (", User.name, ")")' => 'User.name');
			}
			...
			if($this->fparams->get('enable_pm_groups_filter', 0) AND $this->fparams->get('pm_allowed_groups', array())){
				...
				$users = $user->find('list', array('conditions' => array_merge($like, array('Group.group_id' => $this->fparams->get('pm_allowed_groups', array()))), 'fields' => $fields, 'contain' => array('Group', 'User')));
			}else{
				$users = \GCore\Admin\Models\User::getInstance()->find('list', array('conditions' => $like, 'fields' => $fields, 'contain' => array('User')));
			}
			if(!empty($users)){
				foreach($users as $id => $username){
					$json[] = array('value' => $id, 'text' => $username);
				}
			}
		}
	...
	function compose(){
		...
		if(empty($user['id'])){
			...
			\GCore\Libs\Event::trigger('on_permissions_error', r_('index.php?ext=chronoforums&cont=messages'.(!empty($this->data['m']) ? '&act=read&m='.$this->data['m'] : '').'&d='.(!empty($this->data['d']) ? $this->data['d'] : '')));
		...
		if(!empty($this->data['Message'])){
			if(!empty($this->data['cancel_post'])){
				$this->redirect(r_('index.php?ext=chronoforums&cont=messages'.(!empty($this->data['m']) ? '&act=read&m='.$this->data['m'] : '')));
				...
				if($result !== false){
					...
					if((bool)$this->fparams->get('pm_email_notify', 1) === true){
						$this->data['Message']['link'] = \GCore\Helpers\Html::url($this->data['Message']['subject'], r_(\GCore\Libs\Url::root(true).'index.php?ext=chronoforums&cont=messages&act=read&m='.$this->Message->id.'&d='.$did));
					...
					$this->redirect(r_('index.php?ext=chronoforums&cont=messages&act=read&m='.$this->Message->id.'&d='.$did));	
				}else{
	...
	function read(){
		...
		if(!empty($this->data['d'])){
			...
			if(!empty($d_messages)){
				...
				if(!empty($d_messages)){
					//update read status for discussion messages
					$this->Message->MessageRecipient->updateAll(array('opened' => 1), array('message_id' => \GCore\Libs\Arr::getVal($d_messages, array('[n]', 'Message', 'id')), 'recipient_id' => $user['id']));
				}
				if(!empty($d_messages)){
					$this->set(array('d_messages' => $d_messages));
				}
