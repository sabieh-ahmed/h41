...
?>
<?php
if(!empty($this->data['mode']) AND $this->data['mode'] == 'rss'){
	...
	if(!empty($topics)){
		echo '<rss version="2.0">'."\n";
			echo '<channel>'."\n";
				//echo '<title>'..'</title>'."\n";
				//echo '<link>'.\GCore\Libs\Url::current().'</link>'."\n";
				foreach($topics as $topic){
					echo '<item>'."\n";
						echo '<title>'.$topic['Topic']['title'].'</title>'."\n";
						echo '<link>'.r_("index.php?ext=chronoforums&cont=posts&f=".$topic['Topic']['forum_id']."&t=".$topic['Topic']['id']."&p=".$topic['LastPost']['id']."#p".$topic['LastPost']['id'], false, true).'</link>'."\n";
...
?>
	...
	<?php
		...
		if(\GCore\Libs\Request::data('list') == 'user'){
			$list_header = l_('CHRONOFORUMS_MY_TOPICS_HEADER');
			$list_header_info = l_('CHRONOFORUMS_MY_TOPICS_DESC');
		}
		if(\GCore\Libs\Request::data('list') == 'favorites'){
			$list_header = l_('CHRONOFORUMS_MY_FAVORITES_HEADER');
			$list_header_info = l_('CHRONOFORUMS_FAVORITES_DESC');
		}
		if(\GCore\Libs\Request::data('list') == 'tagged'){
			$list_header = sprintf(l_('CHRONOFORUMS_TAGGED_HEADER'), \GCore\Libs\Request::data('tag'));
			$list_header_info = sprintf(l_('CHRONOFORUMS_TAGGED_DESC'), \GCore\Libs\Request::data('tag'));
		}
	?>
	<?php if(!empty($list_header)): ?>
		<h3><?php echo $list_header; ?></h3>
	<?php endif; ?>
	<?php if(!empty($list_header_info)): ?>
		<p><?php echo $list_header_info; ?></p>
	<?php endif; ?>
	...
	<div class="row cfu-topics">
		<form action="<?php echo r_('index.php?ext=chronoforums&cont=topics'); ?>" method="post" name="topictasksform" id="topictasksform">
			<div class="panel panel-default">
				<div class="panel-body">
				<table class="table table-hover table-censored cfu-table">
					<thead class="cfu-head">
						<tr>
							...
							<th class="cfu-check"><?php echo $this->Html->input('topics_ids_checker', array('type' => 'checkbox', 'value' => 1, 'onclick' => 'jQuery(\'#topictasksform\').find(\'.selector_checkbox\').trigger(\'click\');')); ?></th>
							<?php endif; ?>
						</tr>
					</thead>
					<tbody>
						...
						<tr class="cfu-topic<?php if(empty($topic['Topic']['published'])): ?> danger<?php endif; ?><?php if(!empty($topic['Topic']['reported'])): ?><?php endif; ?>">
							...
							<td class="cfu-lastpost">
								<?php if(!empty($topic['Topic']['last_post'])): ?>
									...
									<a href="<?php echo r_("index.php?ext=chronoforums&cont=posts&f=".$topic['Topic']['forum_id']."&t=".$topic['Topic']['id']."&p=".$topic['LastPost']['id']."#p".$topic['LastPost']['id']); ?>" class="gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_VIEW_LATEST_POST'); ?>" alt="<?php echo l_('CHRONOFORUMS_VIEW_LATEST_POST'); ?>"><span class="fa fa-arrow-circle-right"></span></a>
									<span class="cfu-time">
