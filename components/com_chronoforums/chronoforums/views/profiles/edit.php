		...
		<div class="row cfu-data">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form action="<?php echo r_('index.php?ext=chronoforums&cont=profiles&act=edit&u='.$user['User']['id']); ?>" method="post" name="postform" enctype="multipart/form-data" id="postform" class="panel">
					...
					<?php else: ?>
						<?php echo $this->Html->formLine('avatar', array('type' => 'multi', 'label' => l_('CHRONOFORUMS_AVATAR'), 'layout' => 'wide',
							'inputs' => array(
								...
								array('type' => 'custom', 'name' => 'preview', 'code' => $this->Html->image(r_('index.php?ext=chronoforums&cont=profiles&act=avatar&img='.$user['Profile']['params']['avatar']), array('class' => 'img-rounded'))),
							)
						)); ?>
					<?php endif; ?>
					...
					<?php echo $this->Html->formLine('buttons', array('type' => 'multi', 'layout' => 'wide',
						'inputs' => array(
							...
							array('type' => 'submit', 'name' => 'update_profile', 'value' => l_('CHRONOFORUMS_SAVE'), 'class' => 'btn btn-success'),
						)
					)); ?>
