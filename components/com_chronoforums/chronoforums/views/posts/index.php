...
<div class="container">
	...
	<?php if(!empty($posts) OR !empty($keywords)): ?>
		<?php $this->Paginator->url = r_('index.php?ext=chronoforums&cont=posts&f='.$topic['Forum']['id'].'&t='.$topic['Topic']['id'].(!empty($searched) ? "&hilit=".$searched : '')); ?>
		...
		<div class="row cfu-tags">
			<?php if(!empty($tags)): ?>
			<div class="col-md-6 pull-right cfu-tags-form">
				<?php $this->TopicTasks->tags_form($topic, $tags); ?>
			</div>
			<?php endif; ?>
			<?php if((bool)$fparams->get('list_topic_tags', 0) === true): ?>
			<div class="col-md-6 pull-right text-right cfu-tags-list">
			...
			<div class="panel panel-default cfu-head">
				<div class="panel-heading cfu-topic-status">
					<div class="pull-left">
						...
						<?php if((bool)$fparams->get('topic_status_author', 0)): ?>
							<small class="cfu-user-info">
								<i class="fa fa-lg- fa-user"></i><?php echo $this->UserTasks->username(array('User' => $topic['TopicAuthor'], 'Profile' => $topic['TopicAuthorProfile'])); ?>
							</small>
						<?php endif; ?>
					</div>
					<div class="pull-right btn-group-xs">
						...
						<?php $this->TopicTasks->edit_link($topic); ?>
				...
				<div class="panel-body">
					<div class="col-md-4 cfu-topic-buttons">
						...
						<div class="btn-group btn-group-xs">
							...
							<?php $this->TopicTasks->subscribe_button($topic, !empty($subscribed['Subscribed'])); ?>
						</div>
					</div>
	...
	<?php if(!empty($posts)): ?>
		...
		<?php $r_k = 1 - $r_k; ?>
		<?php $ranks_classes = implode(' ', array_map(function($v){return 'cfu-post-by-'.\GCore\Libs\Str::slug($v);}, $this->UserTasks->get_ranks_titles(array('User' => $post['PostAuthor'], 'Profile' => $post['Profile'])))); ?>
		<div class="row cfu-post cfu-bg<?php echo $r_k + 1; ?> <?php echo $ranks_classes; ?>" id="p<?php echo $post['Post']['id']; ?>">
			<?php
				...
				}else if($post['Post']['created'] == $topic['Topic']['created']){//$posts[0]['Post']['id'] AND empty($this->data['posts_loader']) AND $this->Paginator->page == 1){
				...
				<div class="panel-heading cfu-head">
					<div class="container">
						<div class="row">
							...
							<div class="col-lg-<?php echo 12 - $post_title_width; ?> pull-right post-info">
							...
							<div class="col-lg-<?php echo $post_width; ?> cfu-data">
								...
								<div class="btn-group_x btn-group-xs pull-right">
									...
									<?php else: ?>
										...
										<?php $this->PostTasks->split_link($post, $topic); ?>
									...
									<div class="panel-footer">
										<span class="text-right">
											<?php if($fparams->get('enable_pm', 1)): ?>
												<a class="btn btn-default btn-xs gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_SEND_PRIVATE_MESSAGING'); ?>" href="<?php echo r_("index.php?ext=chronoforums&cont=messages&act=compose&u=".$post['PostAuthor']['id']); ?>"><i class="fa fa-envelope fa-lg fa-fw"></i><?php echo l_('CHRONOFORUMS_PM'); ?></a>
											<?php endif; ?>
										</span>
	...
	<?php if(!empty($posts)): ?>
		...
		<div class="row cfu-quickreply">
			<?php $this->TopicTasks->quick_reply($topic); ?>
		</div>
	<?php else: ?>
