...
<div class="container">
	...
	<?php /*if((bool)$fparams->get('enable_home_new_topic', 1) === true): ?>
		<div class="row cfu-header">
	...
	<div class="row cfu-body">
		...
		<?php
			...
			if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $category['Category'])){
				continue;
			}
		...
		<div class="panel panel-default cfu-category">
			<div class="panel-heading cfu-head">
				<h4>
					...
					<a href="<?php echo r_("index.php?ext=chronoforums&c=".$category['Category']['id']); ?>"><?php echo $category['Category']['title']; ?></a>
				...
				<table class="table table-censored table-hover cfu-table">
					...
					<tbody>
						...
						<?php
							...
							if(!\GCore\Extensions\Chronoforums\Chronoforums::_check_perms('display', $forum)){
								continue;
							}
						?>
						<tr class="cfu-row"<?php if((bool)$fparams->get('display_forum_icon', 0) === true): ?> style="background-image: url('<?php echo $this->Output->forum_icon($forum); ?>'); background-repeat: no-repeat; background-position:center left;"<?php endif; ?>>
							<td class="cfu-forum"<?php if((bool)$fparams->get('display_forum_icon', 0) === true): ?> style="padding-left:38px !important;"<?php endif; ?>>
								...
								<?php endif; ?>
								<a href="<?php echo r_("index.php?ext=chronoforums&cont=forums&f=".$forum['id']."&alias=".$forum['alias']); ?>" class="cfu-title gcoreTooltip" title="<?php echo l_('CHRONOFORUMS_READ_POSTS'); ?>"><?php echo $forum['title']; ?></a>
							...
							<td class="cfu-lastpost">
								<span>
									<?php if(!empty($category['LastForumPost'][$k]['id'])): ?>
										...
										<a href="<?php echo r_("index.php?ext=chronoforums&cont=posts&t=".$category['LastForumPost'][$k]['topic_id']."&p=".$category['LastForumPost'][$k]['id']."#p".$category['LastForumPost'][$k]['id']); ?>" title="<?php echo l_('CHRONOFORUMS_VIEW_LATEST_POST'); ?>" alt="<?php echo l_('CHRONOFORUMS_VIEW_LATEST_POST'); ?>" class="gcoreTooltip"><span class="fa fa-arrow-circle-right"></span></a>
										<span class="cfu-time">
