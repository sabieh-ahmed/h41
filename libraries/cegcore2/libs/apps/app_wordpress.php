...
class AppWp {
	...
	function __construct($site = GCORE_SITE, $thread = 'gcore'){
		$this->path = \G2\Globals::get('GCORE_'.strtoupper($site).'_PATH');
		$this->url = \G2\Globals::get('GCORE_'.strtoupper($site).'_URL');
	...
	function dispatch($content_only = false){
		...
		$controller = !empty($this->controller) ? '\C\\'.Str::camilize($this->controller) : '\\'.Str::camilize($this->extension);
		$extension = !empty($this->extension) ? '\E\\'.Str::camilize($this->extension) : '';
		...
		if(empty($J_User->groups) OR empty($G_User['groups']) OR (array_values($J_User->groups) !== $G_User['groups']) OR empty($G_User['inheritance'])){
			...
			if(!empty($J_User->groups)){
				//sort groups
				$groups = \G2\A\M\Group::getInstance()->find('all', array('order' => 'Group.parent_id ASC'));
				if(!empty($groups)){
					reloop:
		...
		*/
		...
		//site title
		\G2\L\Base::setConfig('site_title', get_bloginfo('name'));
	...
	public static function getHeader(){
		...
		foreach($doc->cssfiles as $k => $cssfile){
			if(empty($used['cssfiles'][$k])){
				...
				//$JDocument->addStyleSheet(\G2\Globals::fix_urls($cssfile['href']));
			}
		}
		//add css code list
		...
		//$JDocument->addCustomTag(\G2\Globals::fix_urls($header));
		return \G2\Globals::fix_urls($header);
		//return implode("\n", array_filter($chunks));
	}
	...
	public static function call($site, $extension, $controller = '', $action = '', $params = array()){
		...
		$x_app->set($params);
		$x_app->dispatch(true);
		$tvout = strlen(\G2\L\Request::data('tvout', null)) > 0 ? \G2\L\Request::data('tvout') : '';
		if($tvout != 'ajax'){
			//$doc = \G2\L\Document::getInstance();
