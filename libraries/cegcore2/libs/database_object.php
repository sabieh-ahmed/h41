...
class DatabaseObject {
	...
	public static function getInstance($options = array()){
		if(!empty($options)){
			$db_adapter_class = '\G2\L\DatabaseAdapters\\'.Str::camilize(Config::get('db.adapter', 'pdo'));
	...
	function _prefixTable($sql, $ready = false){
		return str_replace('#__', $this->db_prefix.($ready === false ? \G2\Globals::get('db_table_prefix', '') : ''), $sql);
	}
