...
class Model {
	...
	function pkey(){
		$cached = false;
		if(Config::get('cache.enabled') >= 1 AND Config::get('cache.dbinfo.enabled') >= 1){
			$cache = Cache::getInstance('db_tables_info', array('expiration' => Config::get('cache.dbinfo.lifetime', 43200)), 'file');
		...
		if(!$cached AND Config::get('cache.enabled') >= 1 AND Config::get('cache.dbinfo.enabled') >= 1){
			$cache = Cache::getInstance('db_tables_info', array('expiration' => Config::get('cache.dbinfo.lifetime', 43200)), 'file');
	...
	function tablefields(){
		...
		if(Config::get('cache.enabled') >= 1 AND Config::get('cache.dbinfo.enabled') >= 1){
			$cache = Cache::getInstance('db_tables_info', array('expiration' => Config::get('cache.dbinfo.lifetime', 43200)), 'file');
			$this->tablefields = $cache->get($this->tablename.'.columns');
		...
		if(!$cached AND Config::get('cache.enabled') >= 1 AND Config::get('cache.dbinfo.enabled') >= 1){
			$cache = Cache::getInstance('db_tables_info', array('expiration' => Config::get('cache.dbinfo.lifetime', 43200)), 'file');
			$cache->set($this->tablename.'.columns', $this->tablefields);
		}
	}
	...
	private function _order($fields){
		...
		foreach($fields as $field => $dir){
			$orders[] = $this->quote($field, 'alias').' '.strtoupper($dir);
